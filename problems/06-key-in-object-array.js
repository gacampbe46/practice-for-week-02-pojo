...
function keyInObjectArray(objArray, keyString) {
  for(i = 0; i < objArray.length; i++){ // declare for-loop to iterate through objArray
    let myObj = objArray[i]; // set myObj equal to objArray at index i each time loop runs
    for(key in myObj){ // iterate through each 'key' in 'myObj'
      if(key === keyString || myObj.key == keyString){
        return true; // return true if either they key or the value at key in myObj is equal to 'keyString'
      }
    }
  }
  return false;
